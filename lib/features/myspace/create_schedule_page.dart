...
class _CreateSchedulePageState extends State<CreateSchedulePage> {
  ...
  Future<void> _initCampuses() async {
    ...
    if (_selectedCampusId != null &&
        !_availableCampuses.any((c) => c.campusId == _selectedCampusId)) {
      _selectedCampusId = _availableCampuses.isNotEmpty
          ? _availableCampuses.first.campusId
          : null;
                        ...
                        Expanded(
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<String>(
                              ...
                              items: _weekdays.map<DropdownMenuItem<String>>((
                                String value,
                              ) {
  ...
  Future<void> _saveSchedule() async {
    ...
    try {
      ...
      final overlappingSchedules = currentSchedule.where((s) {
        if (s.id == targetId)
          return false; // Không so sánh với chính môn đang sửa
        if (s.weekday != targetWeekday) return false;

        final sStartMins = (StudyClass.parseTimeToHourFraction(s.start) * 60)
            .round();
        final sEndMins = (StudyClass.parseTimeToHourFraction(s.end) * 60)
            .round();
