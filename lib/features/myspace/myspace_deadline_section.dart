          ...
          Future<void> saveCurrentConfig({bool closeAfterSave = false}) async {
            ...
            if (isEnabled && moodleUrl.isEmpty) {
              ScaffoldMessenger.of(sheetContext).showSnackBar(
                const SnackBar(
                  content: Text('Điền đường dẫn Moodle trước đã.'),
                ),
              );
              return;
            }
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      decoration: _configInputDecoration(
                        context,
                        'vd: https://courses.ctda.hcmus.edu.vn',
                      ),
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: isSaving || isSyncing || !permissionGranted
                                ? null
                                : () async {
                              final moodleUrl =
                              moodleUrlController.text.trim();
          ...
          Expanded(
            child: Text(
              ...
              style: TextStyle(
                ...
                decoration:
                deadline.isCompleted ? TextDecoration.lineThrough : null,
              ),
            ),
          ),
          SizedBox(
    ...
    builder: (dialogContext) {
      return StatefulBuilder(
        builder: (context, setDialogState) {
          return AlertDialog(
            title: const Text('Kết nối Moodle'),
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Tên đăng nhập Moodle',
                  ),
                ),
                ...
                onPressed: isConnecting
                  ...
                  if (username.isEmpty || password.isEmpty) {
                    ScaffoldMessenger.of(dialogContext).showSnackBar(
                      const SnackBar(
                        content: Text('Điền username và password Moodle.'),
                      ),
                    );
                    return;
                  }
                  ...
                  if (token == null || token.trim().isEmpty) {
                    ...
                    ScaffoldMessenger.of(dialogContext).showSnackBar(
                      const SnackBar(
                        content: Text('Không thể kết nối Moodle.'),
                      ),
                    );
                    return;
                  }
