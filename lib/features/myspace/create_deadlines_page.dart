  ...
  void initState() {
    ...
    if (widget.deadline != null) {
      ...
      for (String r in widget.deadline!.reminders) {
        ...
        if (parts.length >= 2) {
          _reminderGroups.add(
            ReminderControllerGroup(value: parts[0], unit: parts[1]),
          );
        }
      }
    } else {
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          focusedBorder: const UnderlineInputBorder(
                            borderSide: BorderSide(color: headerBg),
                          ),
                        ),
                      ),
  ...
  Future<void> _saveDeadline() async {
    ...
    if (_titleController.text.length > 20) {
      AppFeedback.showWarning(
        context,
        'Tên deadline không được vượt quá 20 ký tự',
      );
      return;
    }
    ...
    try {
      ...
      for (var group in _reminderGroups) {
        ...
        int minutesBefore = 0;
        if (unit.contains('phút'))
          minutesBefore = val;
        else if (unit.contains('giờ'))
          minutesBefore = val * 60;
        else if (unit.contains('ngày'))
          minutesBefore = val * 1440;
        else if (unit.contains('tuần'))
          minutesBefore = val * 10080;

        DateTime notificationTime = finalDateTime.subtract(
          Duration(minutes: minutesBefore),
        );
        if (notificationTime.isAfter(DateTime.now())) {
          int nid = (finalDateTime.millisecondsSinceEpoch + minutesBefore)
              .remainder(100000000);
          await NotificationService.scheduleNotification(
            id: nid,
            title: "Nhắc nhở Deadline: ${_titleController.text}",
            body:
                "Deadline của bạn sẽ đến hạn vào lúc ${DateFormat('HH:mm dd/MM').format(finalDateTime)}",
