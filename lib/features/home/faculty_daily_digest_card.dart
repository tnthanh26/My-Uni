...
class FacultyDailyDigestCard extends StatelessWidget {
  ...
  static final Map<String, Map<String, dynamic>> _digestCache = {};
  ...
  Widget build(BuildContext context) {
    ...
    final cachedData = _digestCache[facultyInfo.id];

    return StreamBuilder<DocumentSnapshot>(
      key: ValueKey('digest_stream_${facultyInfo.id}'),
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasData && snapshot.data!.exists) {
          ...
          if (data != null) {
            _digestCache[facultyInfo.id] = data;
          }
        }
        ...
        if (data == null) {
          return StreamBuilder<QuerySnapshot>(
            key: ValueKey('digest_fallback_stream_${facultyInfo.id}'),
            stream: FirebaseFirestore.instance
                .collection('daily_faculty_digest')
            ...
            builder: (context, querySnapshot) {
              if (querySnapshot.hasData && querySnapshot.data!.docs.isNotEmpty) {
                ...
                _digestCache[facultyInfo.id] = latestData;
                return _buildDigestUI(context, latestData, isFallback: true);
              }

              // Nếu có dữ liệu đã cache trước đó, ưu tiên hiển thị ngay mà không hiện banner nháy
              if (cachedData != null) {
                return _buildDigestUI(context, cachedData, isFallback: false);
              }

              // Nếu đang chờ stream kết nối
              if (querySnapshot.connectionState == ConnectionState.waiting) {
                return const SizedBox.shrink();
              }

              // Chỉ khi khẳng định DB thực sự không có digest nào của Khoa này mới hiện banner mặc định
