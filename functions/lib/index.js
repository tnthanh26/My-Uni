...
exports.deleteUserAccountByMod = exports.verifyOTPAndCreateUser = exports.sendRegistrationOTP = exports.cleanupDeletedAccounts = exports.semanticSearch = exports.chatWithUEm = exports.moderateMaterial = exports.moderateReview = exports.moderateForumPost = void 0;
 ...
 */
async function processModeration(snapshot, text) {
...
exports.moderateForumPost = (0, firestore_1.onDocumentCreated)("forum_posts/{docId}", (event) => {
    if (!event.data)
        return null;
    return processModeration(event.data, event.data.data().content || "");
});
exports.moderateReview = (0, firestore_1.onDocumentCreated)("course_reviews/{docId}", (event) => {
    if (!event.data)
        return null;
...
exports.moderateMaterial = (0, firestore_1.onDocumentCreated)("study_materials/{docId}", (event) => {
    if (!event.data)
        return null;
    const data = event.data.data();
 ...
 */
exports.chatWithUEm = (0, https_1.onCall)({ timeoutSeconds: 60, memory: "512MiB" }, async (request) => {
    var _a;
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Bạn cần đăng nhập để hỏi Ú Em.");
    }
    const uid = request.auth.uid;
    const query = request.data.query;
    if (!query || typeof query !== "string") {
        throw new https_1.HttpsError("invalid-argument", "Câu hỏi không hợp lệ.");
    }
    ...
    if (limitDoc.exists) {
        ...
        if ((data === null || data === void 0 ? void 0 : data.lastReset) === today) {
            count = (data === null || data === void 0 ? void 0 : data.count) || 0;
        }
    }
    if (count >= 10) {
        throw new https_1.HttpsError("resource-exhausted", "Bạn đã hết 10 lượt hỏi trong hôm nay. Hẹn gặp lại vào ngày mai!");
    }
    ...
    try {
        const configDoc = await db
            ...
            .doc("chatbot")
            .get();
        const configuredUrl = (_a = configDoc.data()) === null || _a === void 0 ? void 0 : _a.server_url;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Chatbot server error ${response.status}: ${rawBody.substring(0, 500)}`);
        }
        let result;
        try {
            ...
            throw new Error(`Chatbot returned invalid JSON: ${rawBody.substring(0, 500)}`);
        }
        ...
        if (stack)
            console.error("Chatbot stack:", stack);
 ...
 */
exports.semanticSearch = (0, https_1.onCall)({ timeoutSeconds: 60, memory: "512MiB" }, async (request) => {
    var _a, _b;
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Bạn cần đăng nhập để thực hiện tìm kiếm.");
    }
    const { query, scope, tag, sort } = (_a = request.data) !== null && _a !== void 0 ? _a : {};
    ...
    try {
        const configDoc = await db
            ...
            .doc("search")
            .get();
        const configuredUrl = (_b = configDoc.data()) === null || _b === void 0 ? void 0 : _b.server_url;
        if (typeof configuredUrl === "string" && configuredUrl.trim()) {
            serverUrl = configuredUrl.trim();
        }
    }
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Search server error ${response.status}: ${rawBody.substring(0, 500)}`);
        }
        let parsed;
        try {
            ...
            throw new Error(`Search server returned invalid JSON: ${rawBody.substring(0, 500)}`);
        }
        ...
        if (stack)
            console.error("Semantic Search stack:", stack);
 ...
 */
exports.deleteUserAccountByMod = (0, https_1.onCall)(async (request) => {
    var _a, _b, _c, _d;
    // 1. Bắt buộc đăng nhập
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Bạn phải đăng nhập để thực hiện thao tác này.");
    }
    ...
    if (callerEmail !== DELETE_ACCOUNT_MOD_EMAIL) {
        throw new https_1.HttpsError("permission-denied", "Bạn không có quyền xóa tài khoản người dùng.");
    }
    const targetDocumentId = String(((_a = request.data) === null || _a === void 0 ? void 0 : _a.targetUid) || "").trim();
    const requestedTargetEmail = String(((_b = request.data) === null || _b === void 0 ? void 0 : _b.targetEmail) || "").trim().toLowerCase();
    const suppliedReason = String(((_c = request.data) === null || _c === void 0 ? void 0 : _c.reason) || "").trim();
    const reason = suppliedReason ||
        "Mod xóa tài khoản bị từ chối xác thực";
    if (!targetDocumentId && !requestedTargetEmail) {
        throw new https_1.HttpsError("invalid-argument", "Phải cung cấp targetUid hoặc targetEmail.");
    }
    try {
        ...
        if (targetDocumentId) {
            try {
                ...
                if (targetDocument.exists) {
                    ...
                    const emailFromDocument = String(((_d = targetDocument.data()) === null || _d === void 0 ? void 0 : _d.email) || "").trim().toLowerCase();
        ...
        if (targetEmail) {
            try {
                ...
                if (!userQuery.empty) {
                    ...
                    const matchedEmail = String(matchedDocument.data().email || "").trim().toLowerCase();
                    if (matchedEmail) {
        ...
        if (!targetEmail) {
            throw new https_1.HttpsError("not-found", "Không tìm thấy email của tài khoản cần xóa.");
        }
        // Không cho Moderator tự xóa chính mình
        if (targetEmail === callerEmail) {
            throw new https_1.HttpsError("failed-precondition", "Bạn không thể tự xóa tài khoản Moderator.");
        }
        /*
        ...
        try {
            ...
            if (details.code === "auth/user-not-found") {
                throw new https_1.HttpsError("not-found", `Không tìm thấy tài khoản Authentication với email ${targetEmail}.`);
            }
            throw error;
        }
        ...
        try {
            ...
            throw new https_1.HttpsError("internal", `Không thể xóa Firebase Authentication: ${details.code || details.message || "unknown-error"}`);
        }
        /*
         ...
         */
        if (realAuthUid !== firestoreDocumentId) {
            try {
        ...
        throw new https_1.HttpsError("internal", `Không thể xóa tài khoản: ${details.message || "Lỗi không xác định"}`);
    }
});
